  ...
  async.each(outputs.filter(function(x){ return ('save' in x); }), function(output, cb){
    ...
    glob(path.resolve(options.root, output.path), {matchBase: true}, function(err, paths){
      ...
      paths.forEach(function(x){
        ...
        var savePath = (output.save.path && path.resolve(options.root, output.save.path.replace('*', id))) || x
        var name = output.save.resource.replace('*', id)
        var ind;
        for (var i=0; i< dpkg.resources.length; i++){
          if(dpkg.resources[i].name === name){
            ind = i; 
            break;
          }
        }
...
function runTask(task, emitter, callback){
  ...
  var timeoutId = setTimeout(function(){        
    emitter.emit('wrn', util.format('walltime overshoot for task %j', task));
    prog.kill();
  }, task.walltime);
  ...
  prog.on('exit', function (code) {
    ...
    if (code !== 0) {
      ...
      emitter.emit('step', util.format('task %s\nexited with code %d', task.command + ' ' + task.args.join(' '), code));
      callback(null);
    }
  });
