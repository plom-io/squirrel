...
function runTaskSpawn(task, callback){
  ...
  prog.on('exit', function (code) {
    ...
    if(!hasCallbacked){
      hasCallbacked = true;
      if (code !== 0) {
        if( (code === 70) || (code === null) || (code === undefined) ){
          that.emit('error', util.format('task %s\n failed and exited with code %j (keep going)', task.command + ' ' + task.args.join(' '), code));
          callback(null);
        } else {
          callback(new Error(util.format('task %s\nexited with code %j', task.command + ' ' + task.args.join(' '), code)));
        }
      } else {
...
function runTaskExec(task, callback){
  ...
  var child = exec(task.command, task.options, function (err, stdout, stderr) {
    if(err){
      if( (err.code === 70) || (err.code === null) || (err.code === undefined) ){
        that.emit('error', util.format('task %s\n failed and exited with code %j (keep going)', task.command + ' ' + task.args.join(' '), err.code));
        return callback(null);
      } else {
        return callback(err);
      }
    }
    that.emit('step', util.format('task %s\nexited with code 0', task.command));
    return callback(null);
  });
