...
function outResolver(outputs){
  ...
  outputs.forEach(function(out, i){
    if('name' in out){
      ...
      res['$OUT.' + out.name] = out.path; //output must have a path (by definition)
    }
  });
  return res;
};
...
function inResolver(inputs){
  ...
  inputs.forEach(function(inp, i){
    if('name' in inp){
      if((inp._paths.length !== 1) ||  (inp._paths[i].indexOf('*') !== -1)) {
        throw new Error('$IN.' + out.name + ' cannot be used if the path is a glob');
      }
...
function _makeBatchObj(step){
  ...
  var options = (step.cwd) ? {cwd: path.resolve(that.root,step.cwd)}: {};  
  ...
  var outRes = outResolver(step.outputs)
  ...
  if('args' in step){ //->spawn
    command = (step.cwd) ? path.resolve(that.root, step.cwd, step.command): step.command;
